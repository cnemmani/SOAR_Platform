...
def add_cors_headers(response):
    ...
    response.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Tenant-ID, X-User-ID'
...
def login():
    ...
    if user and (user['password_hash'] == hashlib.sha256(password.encode()).hexdigest() or password == user['password_hash']):
        token = hashlib.sha256(f"{username}:{user['role']}:zelarsoar".encode()).hexdigest()
        return jsonify({
            'token': token,
            'user': {
                ...
                'permissions': ['*'] if user['role'] == 'admin' else ['view_own_alerts']
            }
        })
